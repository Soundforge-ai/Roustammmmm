      ...
      <div className="flex items-center gap-1">
        {languages.map((lang) => (
          <button
            ...
            className={`px-2 py-1 text-sm font-medium rounded transition-colors ${locale === lang.code
      ...
      <div className="absolute right-0 top-full mt-1 bg-white rounded-lg shadow-lg border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 min-w-[140px]">
        {languages.map((lang) => (
          <button
            key={lang.code}
            onClick={() => setLocale(lang.code)}
            className={`w-full flex items-center gap-3 px-4 py-2 text-sm text-left hover:bg-gray-50 first:rounded-t-lg last:rounded-b-lg ${locale === lang.code ? 'text-brand-accent font-medium' : 'text-gray-700'
              }`}
          >
            <span>{lang.flag}</span>
