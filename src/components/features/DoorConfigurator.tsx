              ...
              <div className="grid grid-cols-3 gap-4">
                {(['pvc', 'aluminium', 'wood'] as const).map((mat) => (
                  <button
                    ...
                    className={`p-4 rounded-lg border-2 transition-all ${material === mat
              ...
              <div className="space-y-3">
                {DOOR_CLASSES.map((cls) => (
                  <button
                    ...
                    className={`w-full p-4 rounded-lg border-2 text-left transition-all ${doorClass.id === cls.id
                        ? 'border-brand-accent bg-brand-accent/10'
                        : 'border-gray-200 hover:border-gray-300'
                      }`}
                  >
                ...
                <div className="grid grid-cols-5 gap-2 flex-1">
                  {visiblePatterns.map((pattern) => (
                    <button
                      ...
                      className={`aspect-square rounded-lg border-2 overflow-hidden transition-all relative group ${selectedPattern.id === pattern.id
